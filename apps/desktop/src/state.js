...
export function buildSeedData({ projects, tasks }) {
  const seedProjects = projects && projects.length ? projects : [];
  const seedTasks = tasks || [];

  const normalizedProjects = seedProjects.map((project) => ({
    id: sanitizeText(project.id) || `project_${Math.random().toString(16).slice(2)}`,
    name: sanitizeText(project.name) || 'Project',
    color: sanitizeText(project.color) || '#3b82f6',
    archived: Boolean(project.archived)
  }));

  const normalizedTasks = seedTasks.map((task) => normalizeTask(task)).filter(Boolean);

  return {
    projects: normalizedProjects.length ? normalizedProjects : [
...
export function normalizeTask(input = {}, options = {}) {
  ...
  const status = Object.values(TASK_STATUSES).includes(input.status) ? input.status : TASK_STATUSES.Todo;
...
export function upsertTask(tasks = [], draft = {}) {
  ...
  if (tasks.some((task) => task.title.toLowerCase() === validated.title.toLowerCase() && task.projectId === validated.projectId && task.status !== TASK_STATUSES.Deleted)) {
    return {
      ok: false,
...
export function generatePlanSlice(tasks = [], options = {}) {
  ...
  const limit = options.limitMinutes || 720;
